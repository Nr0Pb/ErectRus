...
bool RenderCustomEntryA(CustomEntry *CustomEntryData, OverlaySettingsA CustomSettingsA)
{
	...
	{
		int TextSize = sizeof("-2147483648 хп");
		NameText = new char[TextSize];
		sprintf_s(NameText, TextSize, "%d хп", Health);
	}
	else if (!CustomSettingsA.ShowName && !ShowHealthText && CustomSettingsA.ShowDistance)//Distance
	{
		int TextSize = sizeof("[-2147483648 м]");
		NameText = new char[TextSize];
		sprintf_s(NameText, TextSize, "[%d м]", NormalDistance);
	}
...
bool RenderCustomEntryB(CustomEntry *CustomEntryData, OverlaySettingsB CustomSettingsB)
{
	...
	{
		int DistanceTextSize = sizeof("[-2147483648 м]");
		NameText = new char[DistanceTextSize];
		sprintf_s(NameText, DistanceTextSize, "[%d м]", NormalDistance);
	}
...
void RenderData()
{
	...
	sprintf_s(ScrapLooterText, "Сбор хлама (јвтоматический): %d", int(CustomScrapLooterSettings.ScrapAutomaticLootingEnabled));

	char ItemLooterText[256];
	sprintf_s(ItemLooterText, "Сбор предметов (јвтоматический): %d", int(CustomItemLooterSettings.ItemAutomaticLootingEnabled));

	char NPCLooterText[256];
	sprintf_s(NPCLooterText, "Сбор NPC(Ћимит дистанции 76м) : % d", int(NPCLooterSettings.EntityLooterEnabled));

	char ContainerLooterText[256];
	sprintf_s(ContainerLooterText, "Сбор хранилищ (Ћимит дистанции 6м): %d", int(ContainerLooterSettings.EntityLooterEnabled));

	char HarvesterText[256];
	sprintf_s(HarvesterText, "Сбор флоры (Ћимит дистанции 6м): %d", int(CustomHarvesterSettings.HarvesterEnabled));

	char PositionSpoofingText[256];
	sprintf_s(PositionSpoofingText, "Обман позиции (¬ысота): %d (%d)", int(CustomLocalPlayerSettings.PositionSpoofingEnabled), CustomLocalPlayerSettings.PositionSpoofingHeight);

	char DrawAlphaCodeText[256];
	sprintf_s(DrawAlphaCodeText, "%d %d %d %d %d %d %d %d - Альфа", AlphaCode[0], AlphaCode[1], AlphaCode[2], AlphaCode[3], AlphaCode[4], AlphaCode[5], AlphaCode[6], AlphaCode[7]);

	char DrawBravoCodeText[256];
	sprintf_s(DrawBravoCodeText, "%d %d %d %d %d %d %d %d - Браво", BravoCode[0], BravoCode[1], BravoCode[2], BravoCode[3], BravoCode[4], BravoCode[5], BravoCode[6], BravoCode[7]);

	char DrawCharlieCodeText[256];
	sprintf_s(DrawCharlieCodeText, "%d %d %d %d %d %d %d %d - Чарли", CharlieCode[0], CharlieCode[1], CharlieCode[2], CharlieCode[3], CharlieCode[4], CharlieCode[5], CharlieCode[6], CharlieCode[7]);
...
char **GetFavoritedWeapons()
{
	...
	Array[0] = new char[sizeof("[?] Оружие не выбрано")];
	sprintf_s(Array[0], sizeof("[?] Оружие не выбрано"), "[?] Оружие не выбрано");
	for (int i = 1; i < 13; i++)
	{
		Array[i] = nullptr;
	} 
	...
	for (int i = 1; i < 13; i++)
	{
		if (Array[i] == nullptr)
		{
			Array[i] = new char[sizeof("[?] Недопустимый избранный предмет")];
			sprintf_s(Array[i], sizeof("[?] Недопустимый избранный предмет"), "[%c] Недопустимый избранный предмет", GetFavoriteSlot(BYTE(i - 1)));
		}
	}
