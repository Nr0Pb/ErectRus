	...
	if (Topmost || Layered)
	{
		...
		if (Topmost && !(Style & WS_EX_TOPMOST))
		{
			...
			if (WindowTopmostCounter > 3)
			{
				...
				SetProcessError(0, "Статус процесса: Нет наложения", sizeof("Статус процесса: Нет наложения"));
				OverlayActive = false;
				return false;
			}
		}
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	WindowClassEx.lpszClassName = "ErectRusCLS";
	...
	WindowHwnd = CreateWindowEx(WS_EX_TRANSPARENT | WS_EX_LAYERED, WindowClassEx.lpszClassName, "ErectRus", WS_POPUP, WindowPosition[0], WindowPosition[1], WindowSize[0], WindowSize[1], NULL, NULL, WindowClassEx.hInstance, NULL);
