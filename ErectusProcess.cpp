...
void ResetProcessData(bool ClearProcessError, int NewProcessListSize)
{
	...
	if (NewProcessListSize)
	{
		...
		ProcessList[ProcessIndex] = new char[sizeof("Не выбран процесс")];
		sprintf_s(ProcessList[ProcessIndex], sizeof("Не выбран процесс"), "Не выбран процесс");
		...
		{
			SetProcessError(0, "Статус процесса: Не выбран процесс", sizeof("Статус процесса: Не выбран процесс"));
		}
	}
	else
	{
...
bool HwndValid(DWORD ProcessId)
{
	...
	if (!Pid)
	{
		SetProcessError(2, "Статус процесса: PID (Process Id) ошибка", sizeof("Статус процесса: PID (Process Id) ошибка"));
		return false;
	}
	...
	if (Hwnd == NULL)
	{
		SetProcessError(2, "Статус процесса: HWND (Окно) ошибка", sizeof("Статус процесса: HWND (Окно) ошибка"));
		return false;
	}
	...
	if (Minimized == TRUE)
	{
		SetProcessError(2, "Статус процесса: HWND (Окно) свернуто", sizeof("Статус процесса: HWND (Окно) свернуто"));
		return false;
	}
	...
	{
		SetProcessError(2, "Статус процесса: HWND (Окно) ошибка/свернуто", sizeof("Статус процесса: HWND (Окно) ошибка/свернуто"));
		return false;
	}

	SetProcessError(1, "Статус процесса: Процесс выбран", sizeof("Статус процесса: Процесс выбран"));
	return true;
}
...
bool ProcessValid(DWORD ProcessId)
{
	...
	if (!Exe)
	{
		SetProcessError(2, "Статус процесса: Ошибка базового адресса", sizeof("Статус процесса: Ошибка базового адресса"));
		return false;
	}
	...
	if (Handle == NULL)
	{
		SetProcessError(2, "Статус процесса: HANDLE ошибка", sizeof("Статус процесса: HANDLE ошибка"));
		return false;
	}
