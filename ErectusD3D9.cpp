...
bool D3D9DrawText(const char *Text, bool Shadowed, bool Centered, float *Position, float *Color, float Alpha)
{
	...
	if (Shadowed)
	{
		...
		std::string s;
		s = (const char*)zz;
		wchar_t* BufferShadowed = get_wstr(s);
		D3DXFont->DrawTextW(D3DXSprite, BufferShadowed, -1, &Background, Format, D3DCOLOR_COLORVALUE(0.0f, 0.0f, 0.0f, Alpha));
		delete[]BufferShadowed;
	}
	...
	wchar_t* BufferNormal = get_wstr(s);
	D3DXFont->DrawTextW(D3DXSprite, BufferNormal, -1, &Foreground, Format, D3DCOLOR_COLORVALUE(Color[0], Color[1], Color[2], Alpha));
	delete[]BufferNormal;
